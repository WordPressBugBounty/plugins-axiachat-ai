...
[Read all reviews →](https://wordpress.org/support/plugin/axiachat-ai/reviews/)

== Installation ==

1. Upload the plugin files to the `/wp-content/plugins/axiachat-ai` directory, or install through the WordPress plugin screen.
2. Activate the plugin through the 'Plugins' screen in WordPress.
3. Run the Easy Setup Wizard.
4. Add your AI provider API key.
5. Test your chatbot using the built-in preview — click "Preview Bot" in the bot settings to see it live before deploying globally.
...
Official website:
[axiachat.org](https://axiachat.org/)

Full documentation, tutorials and advanced configuration guides:
[axiachat.org/help](https://axiachat.org/help/)

Community support forum:
[wordpress.org/support/plugin/axiachat-ai](https://wordpress.org/support/plugin/axiachat-ai/)
...
* Voice chat mode (`type="voice_text"`) with browser Web Speech API
* File upload button (📎) with drag-and-drop support, in-chat thumbnail preview, and admin log attachment indicators
...
* Added: Server-side pre-render check — budget limits are evaluated at page load so new visitors never see the AI chat widget when the budget is already exhausted. Admins always see the widget for testing.
* Added: Monthly usage summary in the Usage dashboard — select any month to view total input/output/combined tokens, estimated cost, conversation count, and a per-model breakdown table.
...
= 4.1.1 =
* Added: OpenAI GPT-5.4 family — `gpt-5.4` (1M context, $2.50/$15), `gpt-5.4-mini` (400K, $0.75/$4.50), and `gpt-5.4-nano` (400K, $0.20/$1.25). GPT-5.4 is now the default model for new bots.
* Added: Google Gemini 3.1 models — `gemini-3.1-pro-preview` ($2/$12) and `gemini-3.1-flash-lite-preview` ($0.25/$1.50).
* Updated: Gemini model pricing aligned with current Google API rates (2.5 Pro, 2.5 Flash, 2.5 Flash-Lite, 3 Flash).
* Updated: Gemini 3 Pro Preview marked as shut down (deprecated Mar 9 2026) — use Gemini 3.1 Pro Preview.
...
= 3.0.8 =
* Added: OpenAI GPT-5.3 model support (`gpt-5.3-chat-latest`) — now the default model for new bots.
* Improved: Full page-cache compatibility — automatic cache purge when bot settings change, supporting WP Rocket, LiteSpeed Cache, W3 Total Cache, WP Super Cache, SiteGround Optimizer, WP Fastest Cache, WP-Optimize, Cache Enabler, Hummingbird, Breeze (Cloudways), Kinsta, Cloudflare Super Page Cache, Autoptimize, and Nginx Helper. Third-party plugins can hook into `aichat_after_purge_cache`.
* Added: View and Edit buttons for indexed documents in Training → Context — inspect individual chunks, spot indexing errors, and surgically edit specific chunks with automatic re-embedding on save.
...
* Improved: Markdown-to-HTML fallback now supports image syntax `![alt](url)`.
...
=== 4. Google Gemini (Optional AI Provider) ===
Used for: alternative chat completions via Google Gemini models when a bot provider is set to Google/Gemini.

Endpoint used (HTTPS):
* https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent

Data Sent:
* System instructions + user prompt + condensed prior turns (trimmed) + optional retrieved context snippets.
* Model identifier and generation configuration parameters.

Headers:
* API key passed as URL parameter (`key=`).

Retention (Your Server):
* Same as OpenAI notes for conversation logging.

Legal / Docs:
* Terms: https://ai.google.dev/gemini-api/terms
* Privacy: https://policies.google.com/privacy
* Usage Policy: https://ai.google.dev/gemini-api/terms

Opt‑Out / Control:
* Leave the Gemini API key blank; those bots will fallback/refuse if provider requires it.
* Switch provider per bot to OpenAI or Claude.

=== 5. Freemius SDK (Licensing & Analytics) ===
This plugin uses the Freemius SDK for license activation, update delivery, and anonymous usage analytics (opt-in).

Endpoints used (HTTPS):
* https://api.freemius.com (license checks, updates, analytics)

Data Sent (if opted in):
* Site URL, WordPress version, PHP version, plugin version, and admin email.
* No visitor/chat data is ever sent to Freemius.
...
Opt‑Out / Control:
* During activation, choose "Skip" to opt out of anonymous analytics.
* License-related API calls are required only for premium features.

=== 6. Google Sheets (Optional Lead Export) ===
Only used if you enable the Google Sheets lead destination in the Leads add‑on. The plugin connects to Google APIs via OAuth 2.0 so the site administrator can export captured lead data (name, email, phone, custom fields) to a chosen spreadsheet.

Endpoints used (HTTPS):
* https://accounts.google.com/o/oauth2/v2/auth (OAuth authorization)
* https://oauth2.googleapis.com/token (token exchange & refresh)
* https://www.googleapis.com/oauth2/v2/userinfo (fetch connected account email)
* https://sheets.googleapis.com/v4/spreadsheets/{id} (verify spreadsheet access)
* https://sheets.googleapis.com/v4/spreadsheets/{id}/values/{range}:append (append lead row)
* https://sheets.googleapis.com/v4/spreadsheets/{id}/values/{range} (read/write header row)

Data Sent:
* OAuth flow: authorization code, client credentials, and redirect URI to obtain access & refresh tokens.
* Lead export: one row per captured lead containing the fields the visitor provided (e.g. name, email, phone) plus a timestamp.
* Connected account email is fetched once to display which Google account is linked.

Data Retention:
* Access and refresh tokens are stored encrypted (AES‑256‑CBC) in your WordPress database. Lead data lives in your Google Spreadsheet under your own Google account.
...
Opt‑Out / Control:
* Do not enable the Google Sheets destination in Leads settings — no Google traffic will occur.
* Disconnect your Google account from the Leads → Settings tab to revoke stored tokens.

=== 7. Google Calendar (Optional Appointment Sync) ===
Only used if you enable the Google Calendar adapter in the Appointments add‑on. The plugin connects to Google Calendar API via OAuth 2.0 so the site administrator can sync bookings made through the AI chat with a Google Calendar.

Endpoints used (HTTPS):
* https://accounts.google.com/o/oauth2/v2/auth (OAuth authorization)
* https://oauth2.googleapis.com/token (token exchange & refresh)
* https://oauth2.googleapis.com/revoke (token revocation on disconnect)
* https://www.googleapis.com/oauth2/v2/userinfo (fetch connected account email)
* https://www.googleapis.com/calendar/v3/users/me/calendarList (list available calendars)
* https://www.googleapis.com/calendar/v3/calendars/{id}/events (create, read, update, delete events)

Data Sent:
* OAuth flow: authorization code, client credentials, and redirect URI to obtain access & refresh tokens.
* Event creation: customer name, email, phone, service name, date/time, timezone, optional notes, and a booking code stored as a private extended property.
* Event queries: date range and timezone to check calendar availability before booking.

Data Retention:
* Access and refresh tokens are stored encrypted (AES‑256‑CBC) in your WordPress database. Calendar events live in the administrator’s own Google Calendar account.
...
Opt‑Out / Control:
* Do not enable the Google Calendar adapter in Appointments settings — no Google traffic will occur.
* Disconnect your Google account from the Appointments → Settings tab to revoke stored tokens.

=== 8. Embedded Loader (Your Site's Frontend) ===
...
A: We can build custom capabilities tailored to your business needs. Custom development is available under a project quote. [Contact us for a quote →](https://axiachat.org/)

**Q: Are there paid plans? What do they include?**
A: Yes! AxiaChat AI offers a generous free version with core features. Premium plans unlock additional capabilities. Visit [axiachat.org](https://axiachat.org/) to learn more.

**Q: Is support available?**
A: Free users can get community help via the WordPress.org support forum. Priority support with faster response times is available for paid plan subscribers. Visit [axiachat.org](https://axiachat.org/) for support options. When reporting issues, please include your WP version, PHP version, and logging status. You can also export a diagnostics ZIP from the plugin's Advanced settings to speed up troubleshooting.

== Screenshots ==
1. **Bot Configuration** – Set the AI model, behavior, tone, and appearance for each chatbot independently
2. **Training Context** – Index your WordPress content, WooCommerce products, and PDFs for accurate RAG responses
3. **Training Instructions** – Write or AI-generate the bot’s personality and system prompt with one click
4. **Easy Setup Wizard** – Launch a fully working AI chatbot in under 2 minutes with a guided step-by-step flow
5. **Lead Capture Dashboard** – View, filter, and export leads collected automatically by the chatbot during conversations
6. **Usage & Cost Tracking** – Monitor token consumption, API cost per conversation, and daily usage in real time
7. **Appointment Management** – Review and manage AI-scheduled appointments directly from the WordPress admin panel
